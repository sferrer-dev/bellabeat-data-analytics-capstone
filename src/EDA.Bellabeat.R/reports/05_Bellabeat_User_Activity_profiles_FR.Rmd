...
# Barplot de l'analyse de la dépense calorique selon le niveau d’intensité journalière

```{r barplot-calorie-intensity}
# Catégorisation simple de l'intensité moyenne
df_intensity_cal <- df_users %>%
  mutate(
    intensity_group = case_when(
      mean_intensity < 0.12 ~ "Faible intensité",
      mean_intensity < 0.22 ~ "Intensité modérée",
      TRUE                  ~ "Intensité élevée"
    )
  ) %>%
...
ggplot(df_intensity_cal, aes(x = intensity_group, y = mean_calories, fill = intensity_group)) +
  ...
  labs(
    title = "Calories moyennes par niveau d'intensité journalière",
    x = "Groupe d'intensité",
    y = "Calories moyennes dépensées par jour",
    fill = "Niveau d'intensité"
  ) +
  theme_minimal(base_size = 12) +
  theme(
...
## Plus l’intensité moyenne journalière augmente, plus les calories dépensées chaque jour sont élevées {.unnumbered}

Trois niveaux se distinguent nettement :

* **Intensité élevée** : ~2400 kcal/jour, liée à un volume important de minutes modérées à intenses, signe d’un engagement physique soutenu.
* **Intensité modérée** : ~2100 kcal/jour, profil actif avec moins de pics d’effort, position intermédiaire cohérente avec une activité régulière.
* **Faible intensité** : ~1950 kcal/jour, caractérisée par une majorité de comportements sédentaires et peu d’épisodes d’effort.

L’intensité apparaît ainsi comme un facteur déterminant de la dépense énergétique, au-delà du simple nombre de pas. Cette relation hiérarchisée permet de segmenter les utilisatrices et d’orienter des recommandations axées sur l’intégration d’efforts plus soutenus pour améliorer la dépense calorique quotidienne.
